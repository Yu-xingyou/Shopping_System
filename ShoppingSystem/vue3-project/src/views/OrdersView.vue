  ...
  <div class="orders-container">
    <el-card class="orders-card">
      ...
      <el-table :data="orders" stripe style="width: 100%">
        ...
        <el-table-column label="操作" width="400" fixed="right" v-if="isStaffOrAdmin">
          <template #default="{ row }">
            <el-button
              type="primary"
              size="small"
    ...
    <el-dialog
      ...
      width="450px"
    >
      <div v-if="currentOrderForDiscount" class="discount-info">
        <el-alert
          :title="`订单号：${currentOrderForDiscount.orderNum}`"
          type="info"
          :closable="false"
        ...
        <el-descriptions :column="1" border>
          <el-descriptions-item label="原价">
            <span style="text-decoration: line-through; color: #999;">
              ¥{{ currentOrderForDiscount.originalAmount?.toFixed(2) || currentOrderForDiscount.totalAmount?.toFixed(2) }}
            </span>
          </el-descriptions-item>
          <el-descriptions-item label="当前折扣">
            {{ currentOrderForDiscount.discount ? (currentOrderForDiscount.discount * 10).toFixed(1) + '折' : '无折扣' }}
          </el-descriptions-item>
          <el-descriptions-item label="当前金额">
            <span style="color: #f56c6c; font-weight: bold;">
              ¥{{ currentOrderForDiscount.totalAmount?.toFixed(2) }}
            </span>
          </el-descriptions-item>
        </el-descriptions>

        <el-form :model="discountForm" label-width="100px" style="margin-top: 20px;">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="applyDiscount" :loading="applyingDiscount">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const currentOrderForDiscount = ref(null)
...
const showDiscountDialog = (row) => {
  currentOrderForDiscount.value = row
  discountForm.value.orderId = row.id
  discountForm.value.discount = row.discount || 1.0
  discountDialogVisible.value = true
}

const calculateDiscountedPrice = () => {
  if (!currentOrderForDiscount.value) return '0.00'

  const originalAmount = currentOrderForDiscount.value.originalAmount || currentOrderForDiscount.value.totalAmount
  const discountedPrice = originalAmount * discountForm.value.discount
  return discountedPrice.toFixed(2)
}

const applyDiscount = async () => {
  if (discountForm.value.discount < 0.1 || discountForm.value.discount > 1.0) {
    ElMessage.error('折扣率必须在0.1-1.0之间')
    return
  }

  try {
    applyingDiscount.value = true
    const res = await request.put(`/order/${discountForm.value.orderId}/discount`, null, {
      params: {
    ...
    console.error(error)
    ElMessage.error(error.response?.data?.message || '设置折扣失败')
  } finally {
