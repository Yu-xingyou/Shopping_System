...
public class OrderServiceImpl implements OrderService {
     ...
     * 为订单设置折扣
     * 
     * 验证订单存在性和折扣合法性后，为订单应用折扣。
     * 折扣率范围：0.1-1.0（即1折到全价），如果订单已有折扣，会覆盖原折扣。
     * 系统会自动保存原价并重新计算实际支付金额。
     *
     * @param orderId 订单ID
     * @param discount 折扣率（0.1-1.0之间，如0.8表示8折）
     * @throws BusinessException 当验证失败时抛出业务异常：
     *         - 404: 订单不存在
     *         - 400: 折扣率不合法（必须在0.1-1.0之间）
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void applyDiscount(Integer orderId, BigDecimal discount) {
        log.info("应用订单折扣请求 - orderId: {}, discount: {}", orderId, discount);
        
        Order order = orderMapper.findById(orderId);
        if (order == null) {
            log.warn("应用折扣失败 - 订单不存在: {}", orderId);
            throw new BusinessException(404, "订单不存在");
        }
        
        if (discount == null || discount.compareTo(BigDecimal.valueOf(0.1)) < 0 || discount.compareTo(BigDecimal.ONE) > 0) {
            log.warn("应用折扣失败 - 折扣率不合法: orderId: {}, discount: {}", orderId, discount);
            throw new BusinessException(400, "折扣率必须在0.1-1.0之间");
        }
        
        if (order.getStatus() >= 3) {
            log.warn("应用折扣失败 - 订单已完成或已取消: orderId: {}, status: {}", orderId, order.getStatus());
            throw new BusinessException(400, "已完成或已取消的订单不能设置折扣");
        }
        
        BigDecimal originalAmount = order.getOriginalAmount() != null ? order.getOriginalAmount() : order.getTotalAmount();
        
        orderMapper.updateOrderDiscount(orderId, originalAmount, discount);
        
        log.info("应用订单折扣成功 - orderId: {}, originalAmount: {}, discount: {}, newTotalAmount: {}", 
                orderId, originalAmount, discount, originalAmount.multiply(discount));
    }
