...
public class OrderController {
     ...
     * 为订单设置折扣（仅员工可操作）
     * 
     * 允许员工为订单设置折扣，折扣率范围为0.1-1.0（即1折到全价）。
     * 系统会自动保存原价并重新计算实际支付金额。
     *
     * @param id 订单ID
     * @param discount 折扣率（0.1-1.0之间，如0.8表示8折）
     * @param staffId 员工ID，用于权限验证
     * @return Result 返回操作结果，成功返回折扣后的订单信息
     */
    ...
    public Result applyDiscount(
        ...
        try {
            ...
            Map<String, Object> result = new HashMap<>();
            result.put("orderId", updatedOrder.getId());
            result.put("originalAmount", updatedOrder.getOriginalAmount());
            result.put("discount", updatedOrder.getDiscount());
            result.put("totalAmount", updatedOrder.getTotalAmount());
            
            return Result.success(result);
        } catch (Exception e) {
            return Result.error(500, "设置折扣失败：" + e.getMessage());
        }
    }
